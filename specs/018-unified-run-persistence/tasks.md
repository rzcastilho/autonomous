...
- [X] T067 Implement `prune_preview/1` and `prune/1` facades: preview reports without performing (FR-031e); `prune/1` requires `confirm: true`, executes one transaction per removed run deleting every row across all tables, never removes an in-flight or resumable run in `lib/speckit_orchestrator.ex`, plus a `prune_run/2` transaction in `lib/speckit_orchestrator/store/writer.ex` (depends on T007, T013)
- [X] T068 Emit prune and capacity telemetry events and log them in `Telemetry.attach_default_logger/0` in `lib/speckit_orchestrator/telemetry.ex` (depends on T048, T067)
- [X] T069 [P] `test/speckit_orchestrator/capacity_test.exs` (facade-level) — a refusal blocks `run/1`/`resume/2`/`resume_run/1` only; history/detail/transcript/export/prune remain available (FR-031c, SC-015); hitting the ceiling mid-run drains and halts, discarding nothing (FR-031d)
- [X] T070 [P] `test/speckit_orchestrator/prune_test.exs` (facade-level) — a resumable run is never removed regardless of boundary; preview performs nothing; filling the store past headroom and not pruning leaves row counts unchanged (SC-014)
- [X] T071 [P] `test/speckit_orchestrator/export_test.exs` (facade-level) — exactly one file; store torn down and reconstructed from the file alone with zero external references (SC-016); non-UTF-8 transcript fixture round-trips byte-identically via `"encoding": "base64"`; export mid-run and under a capacity refusal both work and change nothing
...
- [X] T072 Delete `lib/speckit_orchestrator/run_manifest.ex`, `checkpoint.ex`, `transcripts.ex` and their tests `test/speckit_orchestrator/run_manifest_test.exs`, `checkpoint_test.exs`, `transcripts_test.exs` (FR-037) (depends on T032–T048 and T074–T077 having cut over every call site)
- [X] T073 Stop writing `<worktree>/.speckit_logs/` — remove the live transcript copy from `Worktree.commit/2` (research R15, plan Complexity Tracking) in `lib/speckit_orchestrator/worktree.ex`
- [X] T074 [P] Re-point `MissionControlLive` off `RunManifest.read/0` (line ~60) and `Checkpoint.read/1` (line ~67) to `run_detail/1` (live `Coordinator` still wins when active) in `lib/speckit_orchestrator/web/live/mission_control_live.ex`
- [X] T075 [P] Re-point `PipelineDagLive` off `RunManifest.read/0`/`rebuild_layout/2` (lines ~176, ~187) and `Checkpoint.read/2` (line ~194) to `run_detail/1` in `lib/speckit_orchestrator/web/live/pipeline_dag_live.ex`
- [X] T076 [P] Re-point `EscalationsLive` off `RunManifest.read/0`/`rebuild_layout/2` (lines ~86, ~87) and `Checkpoint.read/2` (lines ~182, ~183) to `run_detail/1`'s `escalations` — now the authoritative record, including resolved ones (FR-026) — in `lib/speckit_orchestrator/web/live/escalations_live.ex`
- [X] T077 [P] Re-point `TranscriptsLive` from a directory walk under `<autonomous_root>/transcripts/<segment>/…` to `run_detail/1` for the picker and `transcript/1` to render, and change the feature drawer's transcript link from `?feature=<scope>/<id>&phase=<phase>` to a run-scoped attempt reference in `lib/speckit_orchestrator/web/live/transcripts_live.ex` and `lib/speckit_orchestrator/web/components/feature_drawer.ex`
- [X] T078 Update the five affected console tests — `web/mission_control_live_test.exs`, `web/pipeline_dag_live_test.exs`, `web/escalations_live_test.exs`, `web/reconcile_test.exs`, `web/layout_test.exs` — for facade-sourced data and the new nav entry (depends on T074–T077)
- [X] T079 `test/speckit_orchestrator/web/store_boundary_test.exs` — grep guard: no module under `lib/speckit_orchestrator/web/` references `:mnesia`, `Store.Query`, or `Store.Writer`, and none reads a state file from disk (SC-007, FR-030c) (depends on T056, T063, T074–T077)
- [X] T080 `test/speckit_orchestrator/clean_break_test.exs` — `RunManifest`, `Checkpoint`, `Transcripts`, `Describe.write_pr/3`, `Describe.read_pr/2` no longer exist; a run creates no file under the five paths listed in quickstart.md §10 (FR-037) (depends on T072, T073)
- [X] T081 [P] Update `docs/runbook.md` for the `/runs`/`/runs/:run_id` operator flow, `resumable/1`/`run_history/1`/`run_detail/1`/`transcript/1`, `prune_preview/1`/`prune/1`, `export_run/3`, `store_capacity/0`'s refusal message, and the node-name failure mode (starting under a different node name is a hard failure naming both names, not a silent new store — research R2)
- [X] T082 Run `mise exec -- mix format --check-formatted`, `mise exec -- mix compile` (warnings as errors), `mise exec -- mix test --cover` (pure core >90%), and `mise exec -- mix test --include integration`; confirm every public function on the new `Store.*` modules carries an `@spec` (Principle VI); walk quickstart.md end to end
