...
- [X] T037 [P] Amend `.specify/memory/constitution.md` Principle V's exhaustion bullet (2.2.0 → 3.0.0): permit the advance, preserve the unconditional Critical halt, add the record-and-surface obligation, and prepend a Sync Impact Report over the preserved prior reports
- [X] T038 [P] Update CLAUDE.md, docs/workflow.md, and docs/runbook.md to describe the exhaustion policy and its gate row; leave specs/017-analyze-auto-remediation/ as an accurate historical record
- [X] T039 Run the full gate: `mise exec -- mix format --check-formatted`, `mise exec -- mix compile --warnings-as-errors`, `mise exec -- mix test`, `mise exec -- mix test --cover` (pure core stays >90%). `mix format`/`mix compile --warnings-as-errors` clean; full suite 1174/1175 (1173/1174 tests + 1/1 property), coverage 90.73%. The one failure (`feature_runner_test.exs:711`, "runs without a worktree") is a self-referential artifact of running the suite inside this repo mid-feature — with no `:worktree`, `FeatureRunner` falls back to `Config.repo()` (this checkout), and `.specify/feature.json` currently points at `specs/021-analyze-exhaustion-policy`, so the dry-run's converge step reads *this feature's own* `tasks.md` and finds T040 unchecked (deliberately deferred below). Reproduces identically pre-021 with T030-T040 all unchecked; not a code regression, and resolves itself once this file has zero open boxes. **Superseded**: rather than wait for T040, the test was made hermetic — it now pins `:repo` to a scratch git repo under `System.tmp_dir!/0` (`dry_run_repo/0`), so the dry-run fallback is exercised against a tree the test owns and can no longer read this checkout's `.specify/feature.json`. T040 remains deferred on its own merits.
- [ ] T040 Run quickstart.md Scenario 10 (opt-in, `mise exec -- mix test --include integration`) against a target repo with a persistent, non-mechanically-fixable High finding, confirming unattended completion, the marked PR body, and the console marker. **Deferred** — needs a real target repo and spends real Claude API credits; left as a follow-up manual validation run, tracked the same way Phase 7's LedgerLite validation was.
