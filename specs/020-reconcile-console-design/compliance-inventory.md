...
| run_detail_live.ex | escalation `resolve_escalation/2` button, diverted-feature checkpoint box | §VII.4 / §I.3 | remediated (post-020) | The button read `&check; Resolve` — a friendly synonym that collided with `resolve/1`, the *destructive* full restart on `/escalations`, while the action itself only writes `%{resolved_at, note, by}` onto the escalation record. Relabelled to its real arity and given an `.action-hint`: "records this note against the escalation — does not restart the feature". The page offered no recovery path at all, so a diverted feature's checkpoint box now links to `/escalations#escalation-<id>` labelled `resume/2 from checkpoint` — the same link-out `feature_drawer.ex` already uses, rather than duplicating the guided form (see the run-detail-is-post-mortem note in the moduledoc). |
