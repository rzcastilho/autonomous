defmodule SpeckitOrchestrator.Actions.RunFeaturePhase do
  ...
  * `:converge` → `not_ready?` from the `## CONVERGE: NOT READY` marker.
  ...
  defp classify(:clarify, %PhaseResult{} = r, state, _scope) do
    needs_human? =
  ...
  defp classify(:analyze, %PhaseResult{status: :ok} = r, _state, _scope) do
  ...
  defp classify(:implement, %PhaseResult{status: :ok}, _state, scope) when not is_nil(scope) do
    {:ok, %{}}
  end
  ...
  defp classify(:converge, %PhaseResult{status: :ok} = r, _state, _scope) do
    {:ok, %{not_ready?: Regex.match?(@converge_not_ready_marker, r.final_text || "")}}
  end

  defp classify(_phase, %PhaseResult{} = r, _state, _scope), do: {outcome_of(r), %{}}
