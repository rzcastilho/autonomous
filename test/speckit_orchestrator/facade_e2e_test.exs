defmodule SpeckitOrchestrator.FacadeE2ETest do
  ...
  setup do
    ...
    prev = for k <- [:repo, :breakdown_dir, :worktree_root],
    ...
    File.write!(Path.join(repo, "docs/breakdown/001-core.md"), "# 001\n\n## Prerequisites\n\nNone\n")
  ...
  test "run/0 drives a real one-feature backlog to :done end-to-end (offline)" do
    ...
    refute File.dir?(Path.join(Application.get_env(:speckit_orchestrator, :worktree_root), "001-core"))
  end
  ...
  test "a feature whose worktree can't be created (missing scaffold) is failed" do
    ...
    File.mkdir_p!(Path.join(bare, "docs/breakdown"))
    File.write!(Path.join(bare, "docs/breakdown/001-core.md"), "# 001\n\n## Prerequisites\n\nNone\n")
